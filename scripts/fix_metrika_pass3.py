...
# -*- coding: utf-8 -*-
"""
THIRD PASS: The regex approach missed Metrika blocks that don't have HTML comments.
Some files have 2-3 raw <script> blocks with mc.yandex.ru/metrika code but no
<!-- Yandex.Metrika counter --> comments around them.

Strategy: remove ALL <script> blocks containing mc.yandex.ru/metrika, plus
their associated <noscript> blocks, then insert one clean block.
"""
...
# Match ANY <script> block that contains mc.yandex.ru/metrika
# This catches blocks with or without HTML comments
METRIKA_SCRIPT = re.compile(
    r'<script[^>]*>.*?mc\.yandex\.ru/metrika/tag\.js.*?</script>\s*',
...
# Match any remaining Yandex.Metrika HTML comments (orphaned)
...
def fix_file(filepath):
    ...
    if CLEAN_FAVICON not in content:
        ...
        # Favicon exists, ensure Metrika follows it
